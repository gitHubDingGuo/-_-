...
public class FolderServiceImpl implements IFolderService {
   ...
   private List<FileVo> folderPath(String userId, Integer parentId,List<FileVo> folderLists){
	...
	public List<FileVo> trashLists(String userId, Integer parentId) {
		System.out.println("-----------------trashLists-------------------");
		List<FileVo> fileVoLists = new ArrayList<FileVo>();
		FileVo fileVo;
		Folder_File folder_file;
		Map<String, Object> map, map2;
		...
		List<Folder> folders = folderMapper.trashFolderLists(map);
		System.out.println("------------folder--------");
		for(Folder f:folders) {
			System.out.println(f.toString());
		}
		...
		List<File> files = filemapper.trashFileLists(map2);	
		System.out.println("------------file--------");
		for(File f:files) {
			System.out.println(f.toString());
		}
	...
	@Override
	public List<FileVo> group(String userId, String type,String searchContext) {
		...
		folder_file = new Folder_File();
		map = new HashMap<String, Object>();
		map.put("folder_name", searchContext);
		map.put("userId", userId);
		map.put("status", 1);
		if(type.equals("fileSize")) {
			map.put("type",1);
		}else {
			map.put("type",2);
		}
	   folders = folderMapper.searchFolderLists(map);
			System.out.println("------------folder--------");
			for(Folder f:folders) {
				System.out.println(f.toString());
			}
		...
		files = filemapper.groupFileLists(map);	
			System.out.println("------------file--------");
			for(File f:files) {
				System.out.println(f.toString());
			}
